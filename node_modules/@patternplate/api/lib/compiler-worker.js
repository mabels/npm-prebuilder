...
const send = typeof process.send === "function" && process.connected ? m => process.send(ARSON.stringify(m)) : () => {};
...
function startCompilerWorker() {
  return new Promise(function ($return, $error) {
    var cwd, target, config, compiler, fs;
    let beat, failures;
    cwd = flags.cwd, target = flags.target;
    config = ARSON.parse(flags.config);
    return createCompiler({
      ...
      compiler.plugin("done", stats => {
        if (stats.compilation.errors && stats.compilation.errors.length > 0) {
          stats.compilation.errors.forEach(err => send({
            type: "error",
            target,
            payload: err
          }));
          return send({
            type: "error",
            target,
            payload: stats.compilation.errors
          });
        }
      ...
      process.on("message", envelope => new Promise(function ($return, $error) {
        ...
        switch (message.type) {
          ...
          case "start":
            ...
            return $return(compiler.watch({
              ignored: "**/pattern.json"
            }, () => {}));
