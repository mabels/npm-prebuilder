...
function patternplate(options) {
  return new Promise(function ($return, $error) {
    var port, app, createServer, error, server, clientMiddleware;
    port = options.port;
    app = express();
    createServer = typeof options.server === "string" ? importFrom(options.cwd, options.server) : defaultCreateServer;

    if (typeof options.server === "string" && typeof createServer !== "string") {
      error = new Error(`${options.server} passed via --server must export a function`);
    ...
    return client({
      ...
      clientMiddleware = $await_1;
      app.enable("strict-routing").disable("powered-by").use(errorhandler()).use(clientMiddleware).use(slash());
      return start({
        app,
        port,
        server
