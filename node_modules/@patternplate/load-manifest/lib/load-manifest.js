...
function loadManifest({
  ...
  return new Promise(function ($return, $error) {
    var files, file, isPkg, isPatternPkg, needsPattern;
    let fullPath, data;

    if (typeof cwd !== "string") {
      return $error(new Error(`load-manifest cwd expects string, received ${cwd}, typeof ${cwd}`));
    }

    return map(f => new Promise(function ($return, $error) {
      return sander.exists(cwd, f).then(function ($await_3) {
        return $return($await_3 ? f : null);
      }.$asyncbind(this, $error), $error);
    }.$asyncbind(this))).then(function ($await_4) {
      files = $await_4.filter(Boolean);
      file = files[0];

      if (!file) {
        let err;
        err = new Error(`load-manifest could not find pattern.json, package.json in ${cwd}`);
        err.errno = PATTERNPLATE_ERR_NO_MANIFEST;
        return $error(err);
      }

      fullPath = path.resolve(cwd, file);
      return loadJSON(fullPath).then(function ($await_5) {
        data = $await_5;
        isPkg = path.basename(file) === "package.json";
        isPatternPkg = typeof data.patternplate === "object";
        needsPattern = isPkg && !isPatternPkg;

        if (needsPattern && files.length === 1) {
          let err;
          err = new Error(`load-manifest could not find pattern.json in ${cwd}, package.json contains no patternplate object`);
          err.errno = PATTERNPLATE_ERR_NO_MANIFEST;
          return $error(err);
        }
        ...
        if (needsPattern && files.length === 2) {
          let fullPath, data;
          fullPath = path.resolve(cwd, files[1]);
          return loadJSON(fullPath).then(function ($await_6) {
            data = $await_6;
            return $return({
              file: fullPath,
              manifest: normalize(data, {
                isPatternPkg
              })
            });
          }.$asyncbind(this, $error), $error);
...
function loadJSON(file) {
  return new Promise(function ($return, $error) {
    ...
    try {
      return loadJsonFile(file).then($return, $Try_1_Catch);
    } catch (err) {
      $Try_1_Catch(err)
    }
  }.$asyncbind(this));
