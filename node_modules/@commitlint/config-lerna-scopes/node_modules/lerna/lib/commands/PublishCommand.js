  ...
  _createClass(PublishCommand, [{
    ...
    value: function updateUpdatedPackages() {
      ...
      this.updates.forEach(function (update) {
        ...
        // set new version
        pkg.version = _this7.updatesVersions[pkg.name] || pkg.version;

        // update pkg dependencies
        _this7.updatePackageDepsObject(pkg, "dependencies", exact);
        _this7.updatePackageDepsObject(pkg, "devDependencies", exact);
        _this7.updatePackageDepsObject(pkg, "peerDependencies", exact);
        ...
        if (_this7.options.conventionalCommits) {
          if (_this7.repository.isIndependent()) {
      ...
      if (this.gitEnabled) {
        changedFiles.forEach(function (file) {
          return _GitUtilities2.default.addFile(file, _this7.execOpts);
        });
      }
    }
  }, {
    ...
    value: function gitCommitAndTagVersionForUpdates() {
      ...
      var tags = this.updates.map(function (_ref) {
        ...
        return `${name}@${_this9.updatesVersions[name]}`;
      });
      ...
      tags.forEach(function (tag) {
        return _GitUtilities2.default.addTag(tag, _this9.execOpts);
      });
    ...
    value: function npmPublishAsPrerelease(callback) {
      ...
      this.updates.forEach(function (update) {
        _this10.execScript(update.package, "prepublish");
      });
      ...
      _PackageUtilities2.default.runParallelBatches(this.batchedPackagesToPublish, function (pkg) {
        ...
        var run = function run(cb) {
          ...
          _NpmUtilities2.default.publishTaggedInDir(tag, pkg.location, _this10.npmRegistry, function (err) {
            ...
            err.indexOf("You cannot publish over the previously published version") > -1) {
              ...
              _this10.execScript(pkg, "postpublish");
              cb();
              return;
            }
            ...
            if (attempts < 5) {
              _this10.logger.error("publish", "Retrying failed publish:", pkg.name);
              _this10.logger.verbose("publish error", err);
              run(cb);
            } else {
              _this10.logger.error("publish", "Ran out of retries while publishing", pkg.name, err);
              cb(err);
            }
          });
        };
        ...
        return function (cb) {
          ...
          while (true) {
            ...
            try {
              ...
              if (attempts < 5) {
                _this11.logger.error("publish", "Error updating version as latest", err);
                continue;
              } else {
                cb(err);
                return;
